...
三级菜单不能像二级菜单那么处理,尤其是对于三级菜单单向的DOM 定义,如果包含了三级菜单的二级菜单,会多出一个class属性的subb,并且还有一个sub-x的class属性,通过他定义了它对应的第三级菜单的id值,这样就方便了带有sub属性的二级菜单项定义onmouseover事件时,找到对应的三级菜单项的DOM