...
菜单项在一些网站里是需要动态的定义的,而不是静态的提前定义好的内容,所以,菜单的组成有时需要动态的渲染
...
![动态的加载导航菜单](img/动态的加载导航栏菜单.gif)

### 具体分析

动态加载菜单实际上是将菜单内容保存在一个数组中,实例代码通过对数组遍历两次的方式,把菜单的HTML内容拼接起来,然后挂接在网页的body里,这样就实现了菜单的动态加载功能,在动态网页里,数组这部分的数据往往是通过服务器来生成的
