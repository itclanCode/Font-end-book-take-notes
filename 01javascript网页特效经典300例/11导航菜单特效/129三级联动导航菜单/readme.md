...
与三级导航菜单不同,联动体现了数据的动态性,每一级菜单都是以上一级的数据为基础的，这样的菜单会显得灵活多变
...
![三级联动导航菜单](img/三级联动导航菜单.gif)

### 具体分析

本例的实现还是依赖于数据的动态性,把所有的菜单项都保存在一个数组里,因此可以随时修改这个数组的数据,比如，可以通过动态网页的形式单独提供一个以js为拓展名的文件,它就可以动态的指定所有的菜单项的数据了
